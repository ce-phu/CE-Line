...
public class SaveDataManager
{
    ...
    public static void LoadData()
    {
        ...
        if (string.IsNullOrEmpty(rawData))
        {
            ...
            Player.Data.levelUnlocked[1] = true;
            
            if (Player.Data.starArchived == null || Player.Data.starArchived.Length == 0)
            {
                Player.Data.starArchived = new int[300];
                for (int i = 0; i < 300; i++)
                {
                    Player.Data.starArchived[i] = 0;
                }
            }
        }
        else
